...
import { ChangeStockTypeToItemUsecase } from "./change-stock-type-to-item.usecase"
...
describe("Test ChangeStockTypeToItemUsecase", () => {


    let sut: ChangeStockTypeToItemUsecase
    let props: ChangeStockTypeToItemUsecaseInterface.InputDto
    ...
    beforeEach(() => {
        ...
        sut = new ChangeStockTypeToItemUsecase(stockManagementRepository, eventEmitter)
    })
    ...
    it("Should return StockManagementAlreadyIsItemError if stock is already auto", async () => {
        jest.spyOn(stockManagementEntity, "isStockItem").mockReturnValueOnce(true)
        const output = await sut.execute(props)
        if (output.isRight()) throw new Error("Should not be right")
        expect(output.isLeft()).toBeTruthy()
        expect(output.value[0].name).toBe("StockManagementAlreadyIsItemError")
    })
